...
`process` reports one of four outcomes: `no_task`, `awaiting_owner`, `review`, or `contended`. `contended` means another live invocation is committing — or has just committed — the same completion; this invocation posted nothing, mutated nothing, and released nothing, so it exits 0 and there is nothing to reconcile.
